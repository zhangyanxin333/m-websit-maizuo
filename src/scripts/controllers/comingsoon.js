import comingsoonTpl from "../views/comingsoon.html";
...
const render = async() => {
    let filmList = datasource = (await filmModel.comlist()).data.films;
...
const scroll = ()=>{
    ...
    filmScroll.on("scrollEnd",async function(){
        ...
        if(maxY >= -40 && maxY < 0){
            ...
            let result = await filmModel.comloadmore(++pageNo);
            let list = datasource = [
                ...datasource,
                ...result.data.films
            ]
...
const renderList = (list)=>{
    let template = Handlebars.compile(comingsoonTpl);
